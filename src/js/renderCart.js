    ...
    const cardsCart = cardsData.map((data, i) => {
      ...
      cardCart.innerHTML = `
          ...
          <div class="modal-cart__content">
              <h3 class="modal-cart__title-pizza">${textToLoverToUpperCase(data.name.ru)}</h3>
              <p class="modal-cart__info-pizza"><span class="modal-cart__price-pizza">${data.price[item.size]} ₽</span><span>/</span><span class="modal-cart__weight-pizza">${parseInt(item.size)} см</span></p>
          </div>
          <button class="modal-cart__delete-button" data-id="${item.cardId}"><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
